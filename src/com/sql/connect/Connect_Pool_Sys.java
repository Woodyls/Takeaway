...
public class Connect_Pool_Sys {

	// 创建一个集合用于模拟连接池
	private static LinkedList<Connection> pool_Sys = new LinkedList<Connection>();
	static {
		try {
			// 初始化驱动类
			Class.forName(ConnectInformation.get_DRIVER());
			// 创建10个连接对象
			for (int i = 0; i < DB_Base_Connect_Param.get_Max_Connection_System(); i++) {
				Connection conn = DriverManager.getConnection(ConnectInformation.get_URL(),
						DB_Base_Connect_Param.get_System_ID(), DB_Base_Connect_Param.get_System_PW());
				pool_Sys.add(conn);
			}
	...
	// 取出一个连接
	public synchronized static Connection getConnection(Systems sys, Get_Component component) {

		Connection connect = null;
		// 从队列中移出一个连接对象，返回给调用者
		if (pool_Sys.size() > 0) {
			// 防止为0的时候，导致异常，所以要进行判断
			if (Inquire_User.inquire_person(sys, pool_Sys.get(0), component)) {
				connect = pool_Sys.remove();
			} else {
